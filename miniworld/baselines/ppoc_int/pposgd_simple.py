        ...
        if new:
            ...
            if plots and first_ep:
                if w_intfc:
                    ...
                    if ep_states[0]:
                        ax[0].plot(np.array(ep_states[0])[:,0],np.array(ep_states[0])[:,1], 'r.', zorder=2)                                
                    ...
                    if ep_states[1]:
                        ax[1].plot(np.array(ep_states[1])[:,0],np.array(ep_states[1])[:,1], 'r.', zorder=2)                                
                    plt.colorbar(heatmap,ax=ax[1]) 
                    for a in ax:
                        a.set_xticks([])
                        a.set_yticks([])                   
                    plt.savefig("intfcs/seed{}_iter{}_epi{}.png".format(seed,iters_so_far,ep_num),bbox_inches='tight')
                    plt.clf();plt.close()
...
def add_vtarg_and_adv(seg, gamma, lam,num_options):
    ...
    # seg["tdlamret"] = seg["op_adv"] + seg["op_vpred"]
    ...
    # opts = np.hstack((seg["opts"],np.random.randint(2)))
    opts = seg["opts"]
...
def learn(env, policy_func, *,
    ...
    if wsaves:
        ...
        for i in range(len(files)):
            src = os.path.expanduser('~/baselines_intfc/baselines/ppoc_int/') + files[i]
            dest = os.path.expanduser('~/baselines_intfc/baselines/ppoc_int/') + dirname
    ...
    while True:
        ...
        for opt in range(num_options):
            ...
            if datas[opt] != 0:
                ...
                ###################################################
                elif (indices.size + datas[opt].n > min_batch and datas[opt].n < min_batch):
                    oldmap = datas[opt].data_map
                    cat_ob = np.concatenate((oldmap['ob'],ob[indices]))
                    cat_ac = np.concatenate((oldmap['ac'],ac[indices]))
                    cat_atarg = np.concatenate((oldmap['atarg'],atarg[indices]))
        ...
        if iters_so_far<150 or not fewshot:
            ...
            if w_intfc:
                intgrads = intgrad(seg['ob'],seg['opts'], seg["last_betas"], seg["op_adv"], seg["op_probs"])[0]
                adam.update(intgrads, intlr)

        opgrad = intgrad(seg['ob'],seg['opts'], seg["last_betas"], seg["op_adv"], seg["intfc"])[0]
